...
have bash && safe_run bash --version
have getconf && safe_run getconf GNU_LIBC_VERSION
...
safe_run df -hP "${HOME:-.}"
safe_run df -TP "${HOME:-.}"
have stat && safe_run stat -f -c 'filesystem_type=%T block_size=%S blocks=%b free_blocks=%f available_blocks=%a' "${HOME:-.}"
have findmnt && safe_run findmnt -T "${HOME:-.}" -n -o FSTYPE,OPTIONS
...
have nproc && safe_run nproc --all
have free && safe_run free -h
have lscpu && safe_run bash -lc 'lscpu | grep -E "^(Architecture|CPU\(s\)|Model name|Socket\(s\)|Core\(s\) per socket|Thread\(s\) per core):"'

section "tool availability"
tools=(
  git python3 python pip3 pip rsync rclone scp sftp
  tar gzip zstd xz zip unzip 7z sha256sum md5sum openssl
  curl wget jq sqlite3 cron crontab systemctl docker
  gcc g++ make cmake node npm go rustc cargo ffmpeg
)
...
echo
echo 'Inventory complete. Redact account-specific identifiers before committing raw output.'
